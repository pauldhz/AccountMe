...
class TransactionDTOControllerTest {
    ...
    @Test
    public void shouldReturnAllNLinks() {
        // 6 pages. Current page is 1 -> 1,2,3,4,5
        int SIZE_LIMIT = 1;
        int PAGE_NUMBER = 1;
        ...
        var responseEntity = testRestTemplate.getForEntity(transactionURI, String.class);
        DocumentContext ctx = JsonPath.parse(responseEntity.getBody());
        LinkedHashMap<String, String> nNextLinks = ctx.read("$.links.nNextLinks");
        assertThat(nNextLinks)
                .hasEntrySatisfying("1", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=1"))
                .hasEntrySatisfying("2", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=2"))
                .hasEntrySatisfying("3", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=3"))
                .hasEntrySatisfying("4", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=4"))
                .hasEntrySatisfying("5", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=5"));
    }

    @Test
    public void shouldReturnNRemainingNLinks() {
        // 6 pages. Current page is 3 -> 2,3,4,5,6
        int SIZE_LIMIT = 1;
        int PAGE_NUMBER = 3;
        ...
        assertThat(nNextLinks)
                ...
                .hasEntrySatisfying("4", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=4"))
                .hasEntrySatisfying("5", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=5"))
                .hasEntrySatisfying("6", v -> assertThat(v).isEqualTo("/transactions?size=" + SIZE_LIMIT + "&page=6"));
    }
}